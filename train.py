...
"""Train a 3D GAN using the techniques described in the paper
...
def subprocess_fn(rank, c, temp_dir, desc):
    ...
    if rank == 0:
        # wandb.init(project="eg3d_snarf", name=desc, config=c, entity="eg3d_snarf", settings = wandb.Settings(start_method= 'fork'))
        wandb.init(project="ag3dforzp", name=desc, config=c, entity="742222344", settings = wandb.Settings(start_method= 'fork'))
...
def launch_training(c, desc, outdir, dry_run):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        if c.num_gpus == 1:
            subprocess_fn(rank=0, c=c, temp_dir=temp_dir, desc=desc)
        else:
            torch.multiprocessing.spawn(fn=subprocess_fn, args=(c, temp_dir, desc), nprocs=c.num_gpus)

#----------------------------------------------------------------------------

def init_dataset_kwargs(data,resolution):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.DeepFashionDataset', path=data, use_labels=True, max_size=None, xflip=False, resolution=resolution)
...
@click.option('--test_data',    help='Testing data', metavar='[ZIP|DIR]',                         type=str, default = None)
...
@click.option('--gpc_reg_fade_kimg', help='Length of swapping prob fade', metavar='INT',  type=click.IntRange(min=0), required=False, default=500)
...
@click.option('--restart_every',help='Time interval in seconds to restart code', metavar='INT', type=int, default=-1, show_default=True)

# AG3D additional setting
@click.option('--is_sr_module', help='whether to use superresolution module', type=bool, required=False, default=False)
@click.option('--is_load_noise', help='whether to load pretrained noise module', type=bool, required=False, default=True)
@click.option('--lambda_face', help='weights for the face gan loss', type=float, required=False, default=1)
@click.option('--lambda_eikonal', help='weights for the eikonal loss', type=float, required=False, default=1)
@click.option('--lambda_min_surf', help='weights for the eikonal loss', type=float, required=False, default=0.0)
@click.option('--is_sdf', help='whether to use sdf', type=bool, required=False, default=False)
@click.option('--normal_gan', help='whether to use sdf', type=bool, required=False, default=False)
@click.option('--face_gan', help='whether to use sdf', type=bool, required=False, default=False)
@click.option('--lambda_normal', help='weights for the eikonal loss', type=float, required=False, default=1)
@click.option('--is_normal', help='whether to render normal', type=bool, required=False, default=True)
...
def main(**kwargs):
    ...
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.loss.AG3DLoss')
    c.data_loader_kwargs = dnnlib.EasyDict(pin_memory=True, prefetch_factor=2)

    # Training set.
    c.training_set_kwargs, dataset_name = init_dataset_kwargs(data=opts.data, resolution=opts.image_resolution)
    ...
    if opts.test_data is not None:
        c.testing_set_kwargs, _ = init_dataset_kwargs(data=opts.test_data)
        c.testing_set_kwargs.use_labels = opts.cond
        c.testing_set_kwargs.xflip = opts.mirror
    else: 
        c.testing_set_kwargs = c.training_set_kwargs
    ...
    # Base configuration for AG3D.
    c.ema_kimg = c.batch_size * 10 / 32
    c.G_kwargs.class_name = 'training.triplane.AG3DGenerator'
    c.D_kwargs.class_name = 'training.discriminator.AG3DDiscriminator'
    ...
    rendering_options = {
        ...
        'sr_antialias': True,
        'is_sdf': opts.is_sdf,
        'is_normal': opts.is_normal
    }
        
    if opts.cfg == 'deepfashion':
        rendering_options.update({
            'depth_resolution': 36,
            'depth_resolution_importance': 36,
            'ray_start': -0.3,
            'ray_end': 0.3,
            'box_warp': 1.9,
            'white_back': True,
    ...
    c.loss_kwargs.neural_rendering_resolution_initial = opts.image_resolution//2
    c.loss_kwargs.neural_rendering_resolution_final = opts.image_resolution//2
    c.loss_kwargs.neural_rendering_resolution_fade_kimg = opts.neural_rendering_resolution_fade_kimg
    c.loss_kwargs.lambda_face = opts.lambda_face
    c.loss_kwargs.lambda_eikonal = opts.lambda_eikonal
    c.loss_kwargs.lambda_min_surf = opts.lambda_min_surf
    c.loss_kwargs.normal_gan = opts.normal_gan
    c.loss_kwargs.face_gan = opts.face_gan
    c.loss_kwargs.is_normal = opts.is_normal
    c.loss_kwargs.mbstd_group_size = opts.mbstd_group
    c.loss_kwargs.lambda_normal = opts.lambda_normal
    ...
    c.G_kwargs.is_sr_module = opts.is_sr_module
    
    
    c.D_kwargs.is_sr_module = opts.is_sr_module
    ...
    c.is_load_noise = opts.is_load_noise
